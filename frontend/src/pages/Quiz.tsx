...
const Quiz = () => {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      ...
      setLoading(false);         // ← stop spinner if not authenticated
      return;
    }
    ...
    if (moduleId) {
      const loadQuestions = async () => {
        ...
        await fetchModuleById(moduleId); // ← fetch module on direct navigation
        setLoading(false);
      };
      loadQuestions();
    }
...
//       loadQuestions();
//     }
//   }, [moduleId, isAuthenticated, setShowAuthModal, fetchQuestions, authLoading]);
